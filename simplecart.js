...
function cart(a, b) {
    ...
    this.totalItems = 0, this.totalPrice = 0, this.totalWeight = 0, this.totalShip = 0, this.totalOrder = 0, this.items = new Array, this.userEmail = a, this.ItemColumns = ["Image", "Name", "Price", "Quantity", "Poids", "Total", "Supprimer"], this.initialize = function() {
        ...
        if (!newItem.getValue("name") || !newItem.getValue("price") || !newItem.getValue("Poids")) return alert("Item must have name, weight and price to be added to the cart!"), !1;
        ...
        for (d = getElementsByClassName("simpleCart_items"), a = 0; a < d.length; a++) {
            ...
            for (newRow.className = "cartHeaders", cartTable.appendChild(newRow), a = 0, a = 0; a < this.items.length; a++) {
                for (tempItem = this.items[a], newRow = document.createElement("div"), f = 0, f = 0; f < this.ItemColumns.length; f++) tempCell = document.createElement("div"), tempCell.className = "item" + this.ItemColumns[f], "Image" == this.ItemColumns[f] && tempItem.getValue("image") && (tempCell.innerHTML = '<img src="' + tempItem.getValue("image") + '" />'), "Name" == this.ItemColumns[f] ? tempCell.innerHTML = "<b>" + tempItem.getValue("name") + "</b>" + c(tempItem.getValue("type")) + c(tempItem.getValue("color")) : "Price" == this.ItemColumns[f] ? tempCell.innerHTML = this.returnFormattedPrice(tempItem.getValue("price")) : "Options" == this.ItemColumns[f] && this.options() ? tempCell.innerHTML = tempItem.optionList() : "Quantity" == this.ItemColumns[f] ? tempCell.innerHTML = '<input type="number" onblur="simpleCart.updateQuantity(' + tempItem.functionString() + ",'new_quantity=' + this.value);cekHarga();return false;\"value=\"" + tempItem.getValue("quantity") + '" />' : "Poids" == this.ItemColumns[f] ? tempCell.innerHTML = this.returnFormattedWeight(tempItem.getValue("Poids")) + " g" : "Total" == this.ItemColumns[f] ? tempCell.innerHTML = this.returnFormattedPrice(tempItem.getValue("quantity") * tempItem.getValue("price")) : "Supprimer" == this.ItemColumns[f] && (tempCell.innerHTML = '<a class="removeButton" onclick="simpleCart.updateQuantity(' + tempItem.functionString() + ",'new_quantity=0');cekHarga();return false;\">Supprimer</a>"), newRow.appendChild(tempCell);
